            ...
            checks.forEach(function (check) {
                ...
                _data.read('checks', check, function (err, originalCheckData) {
                    if (!err && originalCheckData) {
                        // Pass it the check validator and let that function continue or log error as needed
...
workers.validateCheckData = function (originalCheckData) {
    ...
    if (originalCheckData.id &&
        ...
        console.log("Error: One of the checks is not properly formatted. Skipping it.", originalCheckData.protocol);
    }
};

// Perform the check and the original and the outcome of the check process, to the next step in the process
                    ...
                    if (!err) {
                        ...
                        _logs.truncate(logId, function (err) {
                            if (!err) {
                                ...
                                console.log("Error trauncating logFile");
                            }
                        });
                    } else {
...
// Timer to execute the worker-process once per minute
workers.logRotationLoop = function (params) {
    setInterval(function () {
        workers.rotateLogs();
    }, 1000 * 60 * 60 * 24);
};
